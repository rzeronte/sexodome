                    ...
                    <div class="thumbnail">
                        <figure>
                            ...
                            @else
                                ...
                                <a href="{{ route('out', ['profile' => App::make('sexodomeKernel')->getSite()->domain, 'scene_id' => $scene->id, 'p' => $i]) }}" rel="nofollow" target="_blank"  class="img link_image" title="{{$scene->title}}" @if (App::make('sexodomeKernel')->getSite()->google_analytics) onclick="trackOutboundLink('{{$href}}', '{{strtolower($scene->channel->name)}}');return false;" @endif  target="_blank">
                                   <span class="thumb-image">
                                        <span class="floater-t-l"><i class="mdi mdi-access-time"></i> {{gmdate("i:s", $scene->duration)}}</span>
                                        <span class="floater-b-l">{{$date->diffForHumans()}}</span>
                                        <img class="img" src="{{$srcThumbnail}}" onmouseout="outThumb(this)" onmouseover="changeThumb(this)" data-thumbs="{{$scene->thumbs}}" data-current-frame="{{$index}}" data-status="stop" alt="{{$scene->title}}"/>
                                    ...
                                    @foreach ($scene->categories()->limit(3)->get() as $category)
                                        ...
                                        <li>
                                            <span class="label label-default">
                                                <a href="{{ route('category', array('profile' => App::make('sexodomeKernel')->getSite()->domain, 'permalink'=> str_slug($translation->name) )) }}" title="Tag">
