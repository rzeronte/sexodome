...
<body style="background-color: @if (ctype_xdigit(Request::input('c', 'ffffff'))) #{{Request::input('c', 'ffffff')}} @else #fffff @endif;">
        <!-- Wrapper for slides -->
    <?php $i = 0 ?>
    <h4> <p><i class="glyphicon glyphicon-th"></i> {{$sexodomeKernel->getSite()->getHost()}}</p></h4>

    <div class="slick">
        @foreach ($categories as $scene)
            <?php $translation = $scene->translations()->where('language_id',$sexodomeKernel->getLanguage()->id)->first(); ?>
            <div class="scene">
                @if ($translation)
                    <a href="{{route('category', ['profile'=>$sexodomeKernel->getSite()->getHost(),'permalink' => $translation->permalink])}}?utm_source={{$sexodomeKernel->getSite()->domain}}&utm_medium={{$translation->name}}&utm_campaign=iframe_sexodome" alt="{{$translation->title}}" target="_blank">
