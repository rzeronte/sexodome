...
@section('pagination_seo')
    @if (isset($pornstars))
        @if ( $pornstars->currentPage() > 1)
            <link rel="prev" href="{{  route('pornstars_page', ['profile' => App::make('sexodomeKernel')->getSite()->getHost(), 'page' => $pornstars->currentPage() - 1]) }} " />
        @endif

        @if ( $pornstars->currentPage() < ($pornstars->lastPage()))
            <link rel="next" href="{{ route('pornstars_page', ['profile' => App::make('sexodomeKernel')->getSite()->getHost(), 'page' => $pornstars->currentPage() + 1])}}" />
        @endif
    @endif
@endsection
