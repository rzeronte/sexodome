            ...
            <form action="{{route('saveTagTranslation', ['locale'=>$locale, 'tag_id' => $tag->id, 'q'=> Request::input("q"), 'page' => Request::input("page")])}}" method="post" class="ajax-form">
                ...
                <div class="col-md-2">
                    ...
                    <select name="status" class="form-control" style="width:70%;">
                        ...
                        <option value="1" <?=($tag->status == '1')?"selected":""?>>Top</option>
                        <option value="2" <?=($tag->status == '2')?"selected":""?>>Index</option>
                    </select>
                </div>

                @foreach ($languages as $lang)
                    <?php $translation = $tag->translations()->where('language_id',$lang->id)->first(); ?>
